...
An interesting thing happens when $\Delta$ is small enough (within a few planetary radii): both North and South poles may not be able to see the object above the horizon (i.e. $\cos(h) > 1$). However, never will both poles see the object above the horizon. This makes sense geometrically.
