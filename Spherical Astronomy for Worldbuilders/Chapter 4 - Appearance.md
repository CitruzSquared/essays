...
In a worldbuilding setting, we can calculate the elongation at times $a$, $b$, and $c$ at any detail we want (not just intervals of days) using the methods from chapters $2$ and $3$. Thus we can repeat this process **to arbitrary precision**.
