...
\end{array}
```
As the planet gets further and further away from the Sun, the time it spends in apparent retrograde motion approaches $T_E/2$ (percentage of time spent in retrograde approaches $50$% as the synodic period approaches $T_E$), which is $182.6\text{ dy}$ for the Earth.\
